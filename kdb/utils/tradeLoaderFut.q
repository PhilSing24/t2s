...
// ============================================================================
// tradeLoaderFut.q - Binance USD-M futures historical per-fill trade loader
//
// Downloads daily individual-trade ZIPs from data.binance.vision (USD-M
// futures archive), extracts the CSV, and loads into a kdb+ partition
// under HDB_BINANCE_DIR. Mirrors tradeLoader.q's structure - same env
// vars, same options, same range workflow.
//
// Difference from aggTradeLoaderFut.q: this loader fetches per-fill
// individual trades rather than aggregated events. Same date can host
// both (different subdirectories under the partition). See ADR-013 for
...
//
// OUTPUT
//   Partitioned splay at HDB_BINANCE_DIR/<date>/trade_fut/
//   Columns: exchTradeTs, exchTradeTimeMs, sym, tradeId, price, qty,
//            quoteQty, buyerIsMaker
//
// NOTES
//   - Files are large: ~3-10x more rows than the equivalent aggTrades
//     archive (every individual fill, not aggregates). Plan accordingly.
//   - Binance USD-M futures trade CSVs have a HEADER ROW; we strip it
//     before parsing.
//   - Boolean column in CSV is "true"/"false" lowercase.
// ============================================================================
...
.cfg.baseUrl: "https://data.binance.vision/data/futures/um/daily/trades/";
...
partitionExists: {[dt]
  / Check the trade_fut subdir specifically (not the parent date dir) so
  / a date with aggTrade_fut already loaded but no trade_fut yet is not
  / treated as already-done by this loader.
  p: raze ((1 _ string .cfg.partitionDir); "/"; string dt; "/trade_fut");
  not () ~ key hsym `$p
 };
...
extractAndClean: {[sym; dt]
  zp: zipPath[sym; dt];
  cp: csvPath[sym; dt];
  / Extract directly to a header-stripped CSV. See aggTradeLoaderFut.q
  / for the rationale; particularly important here because raw per-fill
  / trade files are 3-10x the size of aggTrades files.
  /
  / IMPORTANT: kdb's `system` invokes the command directly via execvp,
  / NOT via a shell. Pipe and redirect operators ('|', '>') are passed
  / as literal arguments to the first program and silently fail. Explicit
  / `/bin/sh -c` wrap is required so the shell parses the operators.
...
// CSV parsing
//
// Binance futures per-fill trade CSV header (verified 2026-06-08):
//   id,price,qty,quote_qty,time,is_buyer_maker
// Types: J F F F J S  (last column is "true"/"false" lowercase, read as
// symbol then converted to boolean below).
// ----------------------------------------------------------------------------

loadTradesFut: {[filepath]
  epochOffset: "j"$1970.01.01D0;
  / Symbol from filename pattern: "BTCUSDT-trades-YYYY-MM-DD.csv"
  ...
  / file into memory). Critical for raw trade files which can be 2-3GB.
  raw: flip
       `tradeId`price`qty`quoteQty`exchTradeTimeMs`buyerIsMaker !
       ("JFFFJS"; ",") 0: filepath;
  select
...
.dl.loadOneDate: {[dt; syms]
  ...
  -1 "  Loading CSVs...";
  t: raze loadTradesFut each paths;
  t: `sym`exchTradeTs xasc t;
  -1 "  Saving partition...";
  .z.zd: (17; 5; 1);
  `trade_fut set t;
  .Q.dpft[.cfg.partitionDir; dt; `sym; `trade_fut];
  delete trade_fut from `.;
  count t
 };
...
downloadAndLoad: {[dt; syms]
  if[partitionExists dt;
    -1 raze ("ERROR: trade_fut partition already exists for "; string dt);
    :()
  ];
...
.dl.range: {[startDt; endDt; syms; opts]
  ...
  while[i < count dates;
    ...
    if[partitionExists dt;
      $[opts `skipExisting;
        [-1 "  trade_fut partition exists - skipping"; skipped: skipped, dt; skipDate: 1b];
...
.dl.usage: {[]
  -2 "Usage:";
  -2 "  q kdb/utils/tradeLoaderFut.q                            (interactive mode)";
  -2 "  q kdb/utils/tradeLoaderFut.q -range START END SYMS [DELAY]";
  -2 "";
  ...
  -2 "";
  -2 "Examples:";
  -2 "  q kdb/utils/tradeLoaderFut.q -range 2026.06.01 2026.06.07 BTCUSDT";
  -2 "  q kdb/utils/tradeLoaderFut.q -range 2026.06.06 2026.06.06 BTCUSDT 5";
 };
...
-1 "  q kdb/utils/tradeLoaderFut.q -range 2026.06.01 2026.06.07 BTCUSDT";
-1 "";
